...
export default function Cuentas() {
  ...
  function guardar(e) {
    ...
    const balance = form.type === "tarjeta_credito" ? -Math.abs(Number(form.balance)) : Number(form.balance);
    dispatch({ type: "ADD_ACCOUNT", payload: { ...form, balance } });
    setForm(emptyForm);
    setModalOpen(false);
  }
    ...
    <div className="flex flex-col gap-5">
      ...
      <div className="grid sm:grid-cols-2 gap-4">
        <Card>
          ...
          <div className="font-display text-2xl font-semibold text-teal">{fmtBs(totalActivos)}</div>
        </Card>
        <Card>
          <div className="text-ink-soft text-xs">Deuda en tarjetas de crédito</div>
          <div className="font-display text-2xl font-semibold text-brick">{fmtBs(totalDeudaTarjetas)}</div>
        </Card>
      </div>

      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        {state.accounts.map((a) => (
          <AccountCard key={a.id} account={a} onDelete={() => dispatch({ type: "DELETE_ACCOUNT", payload: a.id })} />
        ))}
      </div>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title="Nueva cuenta">
        <form onSubmit={guardar} className="flex flex-col gap-3">
          ...
          <Field label={form.type === "tarjeta_credito" ? "Saldo utilizado (Bs)" : "Saldo actual (Bs)"}>
