...
export function SavingsOperationForm({ goalId, initialAmount = "", planMonth = null, onSuccess }) {
  ...
  async function save(event) { event.preventDefault(); setBusy(true); const result=await dispatch({type:"SAVINGS_OPERATION",payload:{...form,amount:Number(form.amount),requestId,planMonth}}); setBusy(false); if(result?.ok === false) setError(result.error); else onSuccess?.(); }
  return <form onSubmit={save} className="flex flex-col gap-3">
    <p className="text-sm text-ink-soft">Este dinero seguirá en tu cuenta o efectivo, pero no aparecerá como disponible para gastar. Apartarlo no es un gasto ni un ingreso.</p>
    ...
    <Field label="Operación"><Select value={form.method} onChange={e=>setForm({...form,method:e.target.value})}><option value="reassign">Reasignar dinero apartado (no es ahorro nuevo)</option><option value="protect">Proteger dinero en la misma cuenta</option><option value="transfer">Transferir dinero libre y apartarlo</option><option value="move">Mover dinero ya apartado a otra cuenta</option><option value="release">Liberar una asignación existente</option><option value="reconcile">Vincular avance anterior sin aumentarlo</option></Select></Field>
    {form.method === "reassign" ? <Field label="Plan desde el que reasignas"><Select value={form.sourceGoalId || state.goals.find(goal=>goal.system==="standard_savings")?.id || ""} onChange={e=>setForm({...form,sourceGoalId:e.target.value})}>{state.goals.filter(goal=>goal.id!==form.goalId).map(goal=><option key={goal.id} value={goal.id}>{goal.name}</option>)}</Select></Field> : null}
    ...
    {["transfer","move"].includes(form.method) ? <Field label="Cuenta real de destino"><Select value={form.destinationAccountId} onChange={e=>setForm({...form,destinationAccountId:e.target.value})}><option value="">Selecciona una cuenta</option>{state.accounts.filter(a=>a.type !== "tarjeta_credito").map(a=><option key={a.id} value={a.id}>{a.name}</option>)}</Select></Field> : null}
    ...
    {form.method === "release" ? <p className="text-xs text-ink-soft">La liberación reduce el dinero apartado y aumenta el disponible en esta misma cuenta. No es un gasto ni dinero utilizado para el objetivo.</p> : null}
...
export function TransferForm({ onSuccess }) {
  ...
  const [transferRequestId]=useState(()=>`transfer-${crypto.randomUUID()}`);
  const [form,setForm]=useState({fromAccountId:accounts[0]?.id || "",toAccountId:accounts[1]?.id || "",amount:""});
  const [error,setError]=useState("");
  async function save(event) { event.preventDefault(); const result=await dispatch({type:"TRANSFER_BETWEEN_ACCOUNTS",payload:{...form,requestId:transferRequestId,amount:Number(form.amount)}}); if(result?.ok === false) setError(result.error); else onSuccess?.(); }
