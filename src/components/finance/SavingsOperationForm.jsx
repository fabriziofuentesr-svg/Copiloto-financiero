...
export function SavingsOperationForm({ goalId, initialAmount = "", onSuccess }) {
  ...
  return <form onSubmit={save} className="flex flex-col gap-3">
    ...
    <Field label="Operación"><Select value={form.method} onChange={e=>setForm({...form,method:e.target.value})}><option value="protect">Proteger dinero en la misma cuenta</option><option value="transfer">Transferir a una cuenta de ahorro y asignarlo</option><option value="release">Liberar una asignación existente</option><option value="reconcile">Vincular avance anterior sin aumentarlo</option></Select></Field>
