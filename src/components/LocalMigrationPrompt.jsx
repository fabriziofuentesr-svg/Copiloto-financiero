...
export function LocalMigrationPrompt() {
  ...
  return <Modal open={visible} onClose={migration.status === "running" ? undefined : dismissMigration} title="Vincular datos de este navegador">
    <div className="flex flex-col gap-4">
      <p>Encontramos datos guardados en este navegador. ¿Quieres vincularlos con tu cuenta?</p>
      ...
      <div className="flex flex-wrap gap-2">
        {migration.status !== "completed" ? <Button disabled={migration.invalid || migration.remoteExists || migration.status === "running"} onClick={importLocal}>{migration.status === "running" ? "Importando…" : migration.status === "failed" ? "Reintentar importación" : "Importar mis datos"}</Button> : null}
        ...
        <Button variant="ghost" disabled={migration.status === "running"} onClick={dismissMigration}>Ahora no</Button>
      </div>
    </div>
  </Modal>;
}
