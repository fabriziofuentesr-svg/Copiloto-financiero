...
export default function PuedoComprarlo() {
  ...
  function evaluar(e) {
    ...
    setResultado(evaluatePurchase(state, amount));
  }
    ...
    <div className="flex flex-col gap-5 max-w-lg">
      ...
      <Card>
        <form onSubmit={evaluar} className="flex flex-col gap-3">
          ...
          <Field label={"Precio (" + (state.profile.currency === "USD" ? "USD" : "Bs") + ")"}>
            <Input type="number" min="0.01" step="0.01" value={precio} onChange={(e) => setPrecio(e.target.value)} required />
          </Field>
      ...
      {resultado && (
        <AlertBanner level={level}>
          ...
          {nombre && <div className="text-xs mt-2 opacity-80">Sobre: {nombre}{precio ? ` (${fmtBs(precio, state.profile.currency)})` : ""}</div>}
        </AlertBanner>
      )}
    </div>
  );
