...
export default function PuedoComprarlo() {
  ...
  return <div className="flex flex-col gap-5 max-w-4xl">
    ...
    {result ? <>
      ...
      <Card title="Supuestos y calidad"><p className="text-sm">Proyección a 30 días después de la compra: <strong>{result.projectedAfter == null ? "No disponible" : fmtBs(result.projectedAfter, state.profile.currency)}</strong></p><p className="text-sm mt-1">Deuda registrada: <strong>{fmtBs(result.debtBalance, state.profile.currency)}</strong> · Calidad de datos: <strong>{result.dataQuality}</strong></p><ul className="list-disc pl-5 text-sm text-ink-soft mt-3">{result.assumptions.map((item) => <li key={item}>{item}</li>)}</ul>{result.missing.length ? <div className="mt-4 rounded bg-paper-raised p-3"><p className="font-medium text-sm">Datos que faltan</p><ul className="list-disc pl-5 text-sm text-ink-soft mt-1">{result.missing.map((item) => <li key={item}>{item}</li>)}</ul><div className="flex gap-3 mt-3"><Link to="/analisis?config=health" className="text-ochre text-sm underline">Configurar análisis</Link><Link to="/cuentas" className="text-ochre text-sm underline">Completar cuentas</Link></div></div> : null}</Card>
    </> : null}
  </div>;
}
