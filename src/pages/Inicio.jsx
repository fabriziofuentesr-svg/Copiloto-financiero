...
function HomeSavingsOverview({state}) {
  ...
  return <Card title="Ahorros y avance de tus planes"><p className="text-sm">Cuentas de ahorro: {fmt(monthlyPlanStatus(state).cash.savings)} · Aportes protegidos en cuentas de uso diario: {fmt(monthlyPlanStatus(state).cash.protectedMoney)}.</p><div className="grid sm:grid-cols-2 gap-4 mt-4">{state.goals.map(goal=><div key={goal.id}><div className="flex justify-between text-sm"><span>{goal.name}</span><span>{goal.system === "standard_savings" ? fmt(goal.current) : `${fmt(goal.current)} de ${fmt(goal.target)}`}</span></div><ProgressBar value={goal.target>0 ? goal.current/goal.target*100 : 0} /></div>)}</div><Link to="/planes-ahorro" className="text-sm underline mt-3 inline-block">Administrar planes y aportes</Link></Card>;
}
