...
export default function Inicio() {
  ...
  return <div className="flex flex-col gap-6">
    ...
    {hasAccounts ? <Card title="Hoy"><div className="grid sm:grid-cols-3 gap-4">{[["Dinero total",actualMonth(state).cash.total],["Disponible para gastar",actualMonth(state).cash.spendable],["Ahorros actuales",actualMonth(state).cash.savings+actualMonth(state).cash.protectedMoney]].map(([label,value])=><p key={label} className="text-sm">{label}: <strong>{fmtBs(value,state.profile.currency)}</strong></p>)}</div><Link className="underline text-sm" to="/cuentas">Ver cuentas</Link></Card> : null}
...
function FinancialSummary({ state, navigate, readiness }) {
  const health = actualFinancialHealth(state);
  ...
  return <div className="flex flex-col gap-5">
    <div className="grid md:grid-cols-2 gap-5">
      ...
      <Card title="Disponible después de compromisos · según tu plan"><div className="font-display text-3xl font-semibold text-teal">{fmtBs(available, state.profile.currency)}</div><p className="text-ink-soft text-xs mt-1">Excluye dinero apartado y compromisos pendientes.</p><div className="mt-4 flex flex-col gap-2 text-sm"><MoneyRow label="Saldo total de activos" value={totalBalance} currency={state.profile.currency} /><MoneyRow label="Apartado en cuentas de ahorro" value={-status.cash.savings} currency={state.profile.currency} /><MoneyRow label="Protegido en otras cuentas" value={-status.cash.protectedMoney} currency={state.profile.currency} /><MoneyRow label="Comprometido" value={-committed} currency={state.profile.currency} /><MoneyRow label="Disponible" value={available} currency={state.profile.currency} strong /></div><Button className="mt-4" size="sm" variant="secondary" onClick={() => navigate("/cuentas")}>Ver y editar cuentas</Button></Card>
      ...
      <Card title="Proyección al cierre · según tu plan">{!projection.available ? <MissingState title="Proyección aún no disponible" text={`Falta: ${projection.missing.join(", ")}.`} onClick={() => navigate("/mi-mes?config=projection")} /> : <><div className="font-display text-3xl font-semibold">{fmtBs(projection.end, state.profile.currency)}</div><p className="text-ink-soft text-sm mt-1">Del {projection.startDate} al {projection.endDate} · {plural(projection.days, "día", "días")} · confianza {projection.confidence}.</p><Link to="/mi-mes" className="text-ochre text-xs underline mt-2 inline-block">Ver fórmula del Plan del mes</Link></>}</Card>
    </div>
    <Card title="Próximos compromisos · según tu plan"><div className="flex flex-col">{commitments.map((item) => <div key={`${item.id}-${item.date}`} className="grid grid-cols-[1fr_auto_auto] gap-3 py-2 border-b border-dotted border-line last:border-none text-sm"><span>{item.name}</span><span className="text-ink-soft">{fmtFecha(item.date)}</span><span className="tabular-nums">− {fmtBs(item.amount, state.profile.currency)}</span></div>)}{commitments.length === 0 ? <p className="text-ink-soft text-sm">No hay compromisos próximos registrados. Añádelos en Mi mes.</p> : null}</div></Card>
    <Card title="Observación · según tu plan"><InsightCard insight={getMainInsight(state)} /><Link to="/mi-mes" className="text-ochre text-xs underline mt-3 inline-block">Ver Mi mes</Link></Card>
  </div>;
}
...
function MonthlyHomeSummary({state}) {
  ...
  return <Card title="Según tu plan"><div className="grid sm:grid-cols-2 gap-4"><div><p className="text-xs text-ink-soft">Proyección al cierre del mes</p><strong className="text-2xl">{fmt(status.closing)}</strong><p className="text-xs mt-1">Objetivo {fmt(status.target)} · {status.targetDifference>=0 ? "por encima" : "por debajo"} en {fmt(Math.abs(status.targetDifference))} · confianza {status.confidence}</p></div><div className="text-xs"><p>Dinero total: {fmt(status.cash.total)}</p><p>Apartado en cuentas de ahorro: {fmt(status.cash.savings)}</p><p>Protegido en otras cuentas: {fmt(status.cash.protectedMoney)}</p><p>Compromisos fijos y cuotas pendientes: {fmt(status.fixedPending+status.debtPending+status.overduePending)}</p><p>Aportes pendientes: {fmt(status.savingsPending)}</p></div></div>{status.alerts.map(item=><p key={item.id} className="text-sm text-brick mt-3">{item.categorySnapshot.name}: {fmt(item.actual)} de {fmt(item.estimated)} · {item.alert === "superado" ? "límite superado" : item.alert === "alcanzado" ? "límite alcanzado" : "cerca del límite"}</p>)}<div className="flex flex-wrap gap-4 mt-4"><Link className="underline text-sm" to="/mi-mes">Revisar mi mes</Link><Link className="underline text-sm" to="/planes-ahorro">Decidir un aporte con dinero disponible</Link></div><p className="text-xs text-ink-soft mt-3">Un saldo positivo al cierre no se asigna automáticamente: conserva el dinero, protégelo en un plan o transfiérelo a ahorro desde Movimientos.</p></Card>;
}

function HomeSavingsOverview({state}) {
  const fmt=value=>fmtBs(value,state.profile.currency);
  return <Card title="Ahorros y avance de tus planes"><p className="text-sm">Apartado en cuentas de ahorro: {fmt(monthlyPlanStatus(state).cash.savings)} · Aportes protegidos en cuentas de uso diario: {fmt(monthlyPlanStatus(state).cash.protectedMoney)}.</p><div className="grid sm:grid-cols-2 gap-4 mt-4">{state.goals.filter(goal=>!goal.archived).map(goal=><div key={goal.id}><div className="flex justify-between text-sm"><span>{goal.name}</span><span>{goal.system === "standard_savings" ? fmt(goal.current) : `${fmt(goal.current)} apartados + ${fmt(goalUsed(state,goal.id))} utilizados de ${fmt(goal.target)}`}</span></div><ProgressBar value={goal.target>0 ? (Number(goal.current)+goalUsed(state,goal.id))/goal.target*100 : 0} /></div>)}</div><Link to="/planes-ahorro" className="text-sm underline mt-3 inline-block">Administrar planes y aportes</Link></Card>;
}
