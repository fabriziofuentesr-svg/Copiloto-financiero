...
import { Button, Card, Field, Input, Modal, Select , MoneyInput } from "../components/ui/primitives.jsx";
...
export default function Movimientos() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <Card title="Historial de ahorro">{state.savingsContributions?.length ? state.savingsContributions.map(item=><p className="text-sm py-2" key={item.id}>{item.date} · {item.goalName || "Fondo de emergencia"} · {item.method === "initial_protection" ? "Ahorro inicial confirmado" : item.method === "move" ? "Trasladado entre cuentas (sin ahorro nuevo)" : item.method === "release" ? "Liberado" : item.method === "reconcile" ? "Avance anterior vinculado" : item.method === "reassign" ? "Reasignado entre planes (sin ahorro nuevo)" : "Aportado"} {fmtBs(item.amount,state.profile.currency)}</p>) : <p className="text-sm text-ink-soft">Los aportes respaldados por cuentas aparecerán aquí.</p>}</Card>
      ...
      <Modal open={Boolean(operation)} onClose={()=>setOperation(null)} title={operation === "saving" ? "Ahorro respaldado por cuentas" : "Transferencia"}>{operation === "saving" ? <SavingsOperationForm goalId={params.get("goalId") || undefined} planMonth={params.get("planMonth") || null} initialAmount={params.get("monto") || ""} onSuccess={()=>setOperation(null)} /> : <TransferForm onSuccess={()=>setOperation(null)} />}</Modal>
...
function RecurringManager() {
  ...
  return <div className="flex flex-col gap-4">
    ...
    <form onSubmit={save} className="border-t border-line pt-4 flex flex-col gap-3"><div className="grid grid-cols-2 gap-3"><Field label="Tipo"><Select value={form.kind} onChange={(event) => setForm({ ...form, kind: event.target.value, category: "" })}><option value="ingreso">Ingreso</option><option value="gasto">Gasto</option></Select></Field><Field label="Día del mes"><Input type="number" min="1" max="31" value={form.dayOfMonth} onChange={(event) => setForm({ ...form, dayOfMonth: event.target.value })} /></Field></div><Field label="Nombre"><Input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} required /></Field><div className="grid grid-cols-2 gap-3"><Field label="Monto"><MoneyInput currency={state.profile.currency} type="number" min="0.01" step="0.01" value={form.amount} onChange={(event) => setForm({ ...form, amount: event.target.value })} required /></Field><Field label="Categoría"><Select value={form.category} onChange={(event) => setForm({ ...form, category: event.target.value })}>{categories.map((category) => <option key={category.id} value={category.id}>{category.name}</option>)}</Select></Field></div><Button type="submit">{editingRecurring ? "Guardar recurrente" : "Añadir recurrente"}</Button>{editingRecurring ? <Button variant="ghost" onClick={() => { setEditingRecurring(null); setForm({ kind: "gasto", name: "", amount: "", dayOfMonth: "1", category: "" }); }}>Cancelar edición</Button> : null}</form>
  </div>;
}
