...
export default function Movimientos() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <Card title="Historial de ahorro">{state.savingsContributions?.length ? state.savingsContributions.map(item=><p className="text-sm py-2" key={item.id}>{item.date} · {item.goalName || "Fondo de emergencia"} · {item.method === "release" ? "Liberado" : item.method === "reconcile" ? "Avance anterior vinculado" : item.method === "reassign" ? "Reasignado desde Ahorros (sin ahorro nuevo)" : "Aportado"} {fmtBs(item.amount,state.profile.currency)}</p>) : <p className="text-sm text-ink-soft">Los aportes respaldados por cuentas aparecerán aquí.</p>}</Card>
